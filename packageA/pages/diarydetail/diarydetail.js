...
Page({
  ...
  onLoad: function(options) {
    ...
    var getData = wx.request({
      url: config.url + '/wx/getwxdiary/' + options.id,
      header: {
      ...
      success: function(res) {
        ...
        that.setData({
          ...
          diarycontent: res.data.html,//给编辑日志用
  ...
  editor(e) {
    ...
    wx.navigateTo({
      url: '../diaryeditor/diaryeditor?id=' + e.currentTarget.dataset.id// + '&title=' + that.data.leassonTilte + '&content=' + that.data.articlecontent
    })
  },
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          ...
          //发起网络请求
          wx.request({
            url: config.url + "/wx/deletewxdiary",
            header: {
              "Content-Type": "application/x-www-form-urlencoded"
            },
            method: "POST",

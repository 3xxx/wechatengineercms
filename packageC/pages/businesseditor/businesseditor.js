// packageC/pages/businesseditor/businesseditor.js
import {
  promisify
...
Page({
  ...
  onLoad: function (options) {
    ...
    if (app.globalData.projectConfig) {
      ...
      wx.request({
        url: config.url + '/wx/getbusinessbyid/' + that.data.id + '?hotqinsessionid=' + sessionId,
        method: 'GET',
        success: function (res) {
          that.setData({
            // processing: res.data
            location: res.data.Location,
            date: `${that.formatDate(res.data.StartDate)} - ${that.formatDate(res.data.EndDate)}`,
            projecttitle: res.data.Projecttitle,
            drivername: res.data.Drivername,
            subsidy: res.data.Subsidy,
            carfare: res.data.Carfare,
            hotelfee: res.data.Hotelfee,
          });
          // console.log(res.data)
          for (var i = 0; i < res.data.BusinessUsers.length; i++) {
            //要增加的数组
            var newarray = [{
              index: i + 1,
              name: res.data.BusinessUsers[i].NickNames.Nickname,
              showtag: true,
            }];
            // tmpimgUrls.push(res.data.articles[i].imgUrl)
            that.setData({
              nameTag: that.data.nameTag.concat(newarray)
            });
          }
        }
      })
  ...
  addName(event) {
    ...
    if (tagindex<this.data.nameTag.length){
      tagindex=this.data.nameTag.length
    }

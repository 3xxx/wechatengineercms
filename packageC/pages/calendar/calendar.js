...
const conf = {
  ...
  onShow: function () {
    ...
    wx.request({
      url: config.url + '/wx/getbusinesscheck',
      method: 'GET',
      data: {
        userid: app.globalData.user_id,
        businessid: that.data.activity_id,
        year: nowyear,
        month: nowmonth,
      },
      header: {
          ...
          qqmapsdk.calculateDistance({
            ...
            success: function (res) {
              ...
              if (dis >= 1000 || dis <= 1000) {
                ...
                var sessionId = wx.getStorageSync('sessionId')
                wx.request({
                  url: config.url + '/wx/businesscheck',
                  method: 'POST',
                  data: {
                    hotqinsessionid: sessionId,
                    userid: app.globalData.user_id,
  ...
  getAddress(latitude, longitude) {
    ...
    qqmapsdk.reverseGeocoder({
      ...
      success(res) {
        ...
        that.setData({
          ad_info: res.result.address,
          city: res.result.address_component.city
        })
      }
    })
    // console.log(that.data.ad_info,'city:',that.data.city)
  },
  // address: "广东省广州市天河区天府路"
  // formatted_addresses:
  // recommend: "广州市天河区人民政府(天府路西)"
  // rough: "广州市天河区人民政府(天府路西)"
  // __proto__: Object
  // address_component:
  // nation: "中国"
  // province: "广东省"
